/* $Id: doscall.c,v 1.31 2022/05/09 14:40:11  Awed Exp $ */

/*
 * Revision 1.31  2022/05/09 14:40:11  Awed
 * Some Bug fix for Win10 VS 2022
 *
 * $Log: doscall.c,v $
...
#if defined(WIN32)
	if ((fp = CreateFileA(p, GENERIC_WRITE | GENERIC_READ, 0, NULL,
...
static long Newfile( char *p, short atr )
{
	 ...
	 * である。
    if ((fp = CreateFileA(p, GENERIC_READ, 0, NULL,
	 ...
	 */
	if ((fp = CreateFileA(p, GENERIC_READ | GENERIC_WRITE, 0, NULL,
						 CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL)) == INVALID_HANDLE_VALUE)
	  {
		  return( -23 );    /* ディスクがいっぱい */
	  }
...
static long Open( char *p, short mode )
{
	...
	switch( mode ) {
	  ...
	  case 1: /* 書き込みオープン */
#if defined(WIN32)
		if ((fh = CreateFileA(p, GENERIC_READ, 0, NULL,
							 OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL)) == INVALID_HANDLE_VALUE)
		  return -2;
		CloseHandle(fh);
		md = GENERIC_WRITE;
